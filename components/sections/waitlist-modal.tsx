...
export const WaitlistModal: React.FC<WaitlistModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Send request to Next.js serverless API route (/api/waitlist)
      const res = await fetch('/api/waitlist', {
        ...
        body: JSON.stringify({
          ...
          track,
          businessName: track === 'merchant' ? businessName.trim() : undefined,
          category: track === 'merchant' ? category : undefined,
        }),
      });
      ...
      setQueuePosition(result.data?.queuePosition || 1340);
      setIsLoading(false);
      setIsSuccess(true);
    } catch (err) {
      console.error('Waitlist API submission error:', err);
